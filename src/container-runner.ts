...
function buildVolumeMounts(
  ...
  // Always sync typing hooks so the host can track agent activity and keep
  // the Telegram typing indicator alive for long-running requests.
  // PreToolUse/PostToolUse write a Unix timestamp; Stop writes "stopped".
  settings.hooks = {
    PreToolUse: [
      {
        matcher: '.*',
        hooks: [
          {
            type: 'command',
            command: 'date +%s > /workspace/ipc/typing_heartbeat',
            timeout: 5,
          },
        ],
      },
    ...
    Stop: [
      {
        ...
        hooks: [
          {
            type: 'command',
            command: "printf 'stopped' > /workspace/ipc/typing_heartbeat",
            timeout: 5,
          },
        ],
      },
